...
description: "In-memory state for temporary UI data without persistence"
type: "integration-guide"
tags: ["tanstack", "db", "localonly-collection", "in-memory", "ui-state", "temporary", "typescript"]
category: "typescript"
...
status: "stable"
sources:
  - name: "LocalOnly Collection Docs"
...
related: ["../localstorage/guide.md", "../query/guide.md", "../../concepts.md", "../../frameworks/react/guide.md"]
author: "unknown"
contributors: []
...
LocalOnly collections are designed for in-memory client data or UI state that doesn't need to persist across browser sessions or sync across tabs. ([LocalOnly Collection Docs][1])

## Overview

The `localOnlyCollectionOptions` allows you to create collections that: ([LocalOnly Collection Docs][1])
...
**Important:** LocalOnly collections work differently than server-synced collections. With LocalOnly collections, you **directly mutate state** by calling methods like `collection.insert()`, `collection.update()`, and `collection.delete()` - that's all you need to do. The changes are immediately applied to your local in-memory data. ([LocalOnly Collection Docs][1])
...
- `id`: Unique identifier for the collection ([LocalOnly Collection Docs][1])
...
- [LocalStorage Collection](./localstorage-collection.md)

## References

[1]: https://github.com/TanStack/db/blob/main/docs/collections/local-only-collection.md "TanStack DB LocalOnly Collection Documentation"
