...
description: "Persistent local storage with cross-tab synchronization"
type: "integration-guide"
tags: ["tanstack", "db", "localstorage-collection", "localstorage", "persistence", "cross-tab", "typescript"]
category: "typescript"
...
status: "stable"
sources:
  - name: "LocalStorage Collection Docs"
...
related: ["../localonly/guide.md", "../query/guide.md", "../../concepts.md", "../../frameworks/react/guide.md"]
author: "unknown"
contributors: []
...
LocalStorage collections store small amounts of local-only state that persists across browser sessions and syncs across browser tabs in real-time. ([LocalStorage Collection Docs][1])

## Overview

The `localStorageCollectionOptions` allows you to create collections that: ([LocalStorage Collection Docs][1])
...
**Important:** LocalStorage collections work differently than server-synced collections. With LocalStorage collections, you **directly mutate state** by calling methods like `collection.insert()`, `collection.update()`, and `collection.delete()` - that's all you need to do. The changes are immediately applied to your local data and automatically persisted to localStorage. ([LocalStorage Collection Docs][1])
...
- `id`: Unique identifier for the collection ([LocalStorage Collection Docs][1])
...
- `storageEventApi`: Event API for subscribing to storage events (defaults to window). Enables custom cross-tab, cross-window, or cross-process synchronization ([LocalStorage Collection Docs][1])
...
**Important:** LocalStorageCollections auto-hydrate from localStorage on creation. Don't insert default records during app initialization - it races with hydration and overwrites persisted data. ([LocalStorage Collection Docs][1])

**Pattern:** Insert default records lazily (on first mutation), not eagerly (on app startup).

```typescript
// BAD - races with hydration
const collection = createCollection(localStorageCollectionOptions({...}))
collection.insert({ id: 'default', value: 'initial' }) // May overwrite persisted data!

// GOOD - lazy initialization
function ensureDefaults() {
  if (collection.get('default') === undefined) {
    collection.insert({ id: 'default', value: 'initial' })
  }
}
```
...
The `storageEventApi` option allows the collection to subscribe to storage events for cross-tab synchronization:
...
const syncedCollection = createCollection(
  localStorageCollectionOptions({
    ...
    storageEventApi: customStorageEventApi,
    getKey: (item) => item.id,
  })
)
```

This enables synchronization across:
...
When using LocalStorage collections with manual transactions (via `createTransaction`), you must call `utils.acceptMutations()` to persist the changes:
...
- [LocalOnly Collection](./localonly-collection.md)

## References

[1]: https://github.com/TanStack/db/blob/main/docs/collections/local-storage-collection.md "TanStack DB LocalStorage Collection Documentation"
