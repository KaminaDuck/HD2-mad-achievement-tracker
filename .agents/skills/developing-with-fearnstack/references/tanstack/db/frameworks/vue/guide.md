---
title: "TanStack DB Vue Guide"
description: "Vue composables for reactive live queries with TanStack DB"
type: "framework-guide"
tags: ["tanstack", "db", "vue", "composables", "useLiveQuery", "typescript"]
category: "typescript"
...
([Vue Adapter Overview][1])

## Vue Composables

For comprehensive documentation on writing queries (filtering, joins, aggregations, etc.), see the [Live Queries Guide](./concepts.md). ([Vue Adapter Overview][1])
...
The `useLiveQuery` composable accepts an optional dependency array as its last parameter. When any reactive value in the array changes, the query is recreated and re-executed. ([Vue Adapter Overview][1])
...
import { useLiveQuery } from '@tanstack/vue-db'
import { eq, and, like } from '@tanstack/db'
...
const { data: todos, isLoading, status } = useLiveQuery(
  (q) => {
    let query = q.from({ todo: todosCollection })

    // Apply filters based on reactive values
    ...
    if (searchTerm.value) {
      query = query.where(({ todo }) =>
        like(todo.text, `%${searchTerm.value}%`)
      )
    }
...
<template>
  <div class="todo-app">
    ...
    <form @submit.prevent="addTodo($event.target.text.value)">
...
### useLiveQuery Return Values

| Property | Type | Description |
|----------|------|-------------|
| `data` | `ComputedRef<T[]>` | The reactive query results |
| `isLoading` | `ComputedRef<boolean>` | True during initial data fetch |
| `status` | `ComputedRef<string>` | Query status ('pending', 'ready', 'error') |
| `collection` | `LiveQueryCollection` | The underlying collection (when using pre-created) |

## Learn More

- [Live Queries Guide](./concepts.md)
- [React Adapter](./react-guide.md)
- [Mutations Guide](./concepts.md)

## References

[1]: https://github.com/TanStack/db/blob/main/docs/framework/vue/overview.md "TanStack DB Vue Adapter Overview"
